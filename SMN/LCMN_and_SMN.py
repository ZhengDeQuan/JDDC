...
GPU_number = 0
os.environ["CUDA_VISIBLE_DEVICES"] = str(GPU_number)       # 使用第一块GPU
...
class SCN():
    def __init__(self, embedding):
        self.max_num_utterance = 20
        self.negative_samples = 2
        self.max_sentence_len = 60
        self.word_embedding_size = 200
        self.rnn_units = 200
        self.total_words = embedding.shape[0]  # 136365
        self.batch_size = 128
        self.print_batch = 200
    ...
    def train_model_with_random_sample(self, continue_train=False, previous_model_path="model"):
        ...
        config = tf.ConfigProto()
        config.gpu_options.per_process_gpu_memory_fraction = 0.96  # 只分配40%的显存
...
def train_onehotkey():
    ...
    print('start train')
    sys.stdout.flush()
    scn.train_model_with_random_sample(continue_train=True, previous_model_path="model/model.9")
