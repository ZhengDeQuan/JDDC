...
class seq2seq:
    def __init__(self,vocab_size,learning_rate,encoder_size,max_length,embedding_size,sos_token,eos_token,unk_token,beam_size=5):
        ...
        with tf.variable_scope("seq2seq-train"):
            ...
            train_decoder=BasicDecoder(attention_cell,train_helper,initial_state=decoder_initial_state_beam,output_layer=decode_out_layer)
        ...
        with tf.variable_scope("seq2seq_beam_search_generate"):
            ...
            beam_infer_decoder = BeamSearchDecoder(attention_cell, embedding=word_embedding, end_token=self.EOS_token,
                                              start_tokens=start_tokens, initial_state=decoder_initial_state_beam,
        ...
        with tf.variable_scope("greedy_generate"):
            ...
            inference_decoder = tf.contrib.seq2seq.BasicDecoder(cell=attention_cell, helper=decoding_helper,
    ...
    def load_fasttext_embedding(self):
        ...
        with open(self.embedding_file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                templist = [float(s) for s in strs[1:]]
                if len(templist)==100:
...
def preprocess():
    train_data = prepare_data(query_file='./data/train.enc',
                              reply_file='./data/train.dec',
                              target_file='./data/train.dec',
                              embedding_file='./data/model.vec')
    pickle.dump(train_data, open('./data/train.pkl', 'wb'), protocol=True)
    val_data = prepare_data(query_file='./data/val.enc',
                              reply_file='./data/val.dec',
                              target_file='./data/val.dec',
                            embedding_file='./data/model.vec')
    pickle.dump(val_data, open('./data/val.pkl', 'wb'), protocol=True)
    test_data = prepare_data(query_file='./data/test.enc',
                              reply_file='./data/test.dec',
                              target_file='./data/test.dec',
                            embedding_file='./data/model.vec')
...
def train_onehotkey(beam_size=5,batch_size=128,n_epochs=6,batches_per_evaluation=500,previous_model_path=None,continue_train=False):
...
if __name__=='__main__':
    # preprocess()
    train_onehotkey(batch_size=32,beam_size=1,n_epochs=10,continue_train=False)
    generate_batches(beam_size=5,output_path='./output/beamsearch_result',model_path='./model/epoch.10.model',batch_size=32,use_beam_search=False)
